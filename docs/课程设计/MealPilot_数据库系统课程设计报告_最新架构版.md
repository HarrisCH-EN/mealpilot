...
本文按照《数据库系统课程设计》任务书要求整理，内容以 MealPilot（饭有谱）当前最新实现为准。报告重点放在需求分析、概念结构、逻辑结构、数据完整性、事务处理、推荐数据加工以及系统实现上。生产小程序已经从“公网域名直连 Backend”调整为 CloudBase 托管调用：普通业务请求通过 `wx.cloud.callContainer` 进入 `mealpilot-api`，图片则采用 Backend prepare、`wx.cloud.uploadFile` 写入 staging、Backend commit 下载校验并转存正式目录的两阶段流程。数据库主体结构没有因此改变，仍为 20 张业务表和 29 条外键。

系统功能模块图、总体 E-R 图、总体架构图、菜谱推荐处理流程图以及运行截图在本文中保留占位符，后续可直接替换为最终导出的图片。
...
10. 将后端、MySQL 和对象存储部署到 CloudBase，并使用 `wx.cloud.callContainer` 与 `wx.cloud.uploadFile` 组成生产通信链路，使小程序不依赖默认公网测试域名完成主要业务。
...
8. **媒体文件管理**：头像和菜谱封面保存在 CloudBase 私有 Storage 中。生产环境先由后端签出受用户或家庭范围约束的 staging 路径，小程序使用 `wx.cloud.uploadFile` 上传，后端再下载校验文件大小与真实图片格式，转存正式目录并把稳定 File ID 写入数据库。

> **图 1-1 系统功能模块图（后续补充）**

## 1.5 非功能需求

除了功能本身，系统在实现时还考虑了安全性、可维护性和异常恢复。生产环境关闭开发登录，微信密钥、JWT 密钥、数据库密码和 CloudBase API Key 都只保存在服务端；普通业务请求通过 `wx.cloud.callContainer` 进入指定云托管服务，页面层不直接拼接生产域名。JWT 仍然作为 MealPilot 自身的用户会话凭证，和 CloudBase 的服务寻址职责分开。

图片上传没有把大文件塞进普通 API 请求，而是使用 CloudBase 的专用上传能力。后端先生成 staging 路径并登记延迟清理任务，小程序上传后只把 File ID 交回后端；后端再执行路径归属、5MB 上限和 JPEG/PNG/WebP magic bytes 校验，验证通过后才转存到正式目录。这样即使用户中途退出或 commit 失败，staging 文件也可以由 `storage_cleanup_jobs` 在后续清理。

涉及多个数据表的操作尽量放在同一事务中，避免只写入一半数据；推荐批次会保留当时的请求参数、候选分数和推荐理由，家庭解散也不会立即清空所有数据，而是先进入归档状态并保留恢复期。这些设计主要是为了让系统在出现异常时仍然有迹可循，而不是只追求“页面能够打开”。
...
生产后端服务名为 `mealpilot-api`，运行在腾讯 CloudBase 云托管。MySQL 保存结构化业务数据，CloudBase 私有 Storage 保存头像和菜谱封面。数据库中只持久化稳定的 `cloud://` File ID，真正展示图片时再由后端调用 Storage SDK 生成临时 HTTPS URL 返回给小程序。

生产环境的普通 API 不再依赖 CloudBase 默认公网测试域名，而是由小程序通过 `wx.cloud.callContainer` 指定环境 ID 和服务名进行调用。图片走另一条通道：prepare/commit 仍通过 `callContainer`，文件本体则由 `wx.cloud.uploadFile` 直接上传到 Storage 的 staging 目录，随后由 Backend 下载、校验并转存。开发环境为了便于调试，仍保留 localhost 的 `wx.request` 和 `wx.uploadFile`。
...
项目通过 Git 和 GitHub 管理版本。后端和小程序都保留了自动化测试，其中后端重点覆盖认证、家庭边界、事务、推荐、Storage 和 staged upload，小程序测试主要验证登录流程、页面状态、cloud transport、401 重认证以及 `prepare → uploadFile → commit` 的前端契约。涉及真实 MySQL 写入的集成测试使用独立测试库和显式写入开关，避免误操作生产数据库。

开发环境和生产环境使用不同 transport。开发环境继续访问 `127.0.0.1:3000`；生产环境要求 `transport=cloud`，并配置 CloudBase 环境 ID 和 `mealpilot-api` 服务名。生产启动时还会检查 JWT、微信登录和 CloudBase Storage 所需的关键环境变量，并强制关闭开发登录。密码、AppSecret 和 API Key 等敏感信息只存在于服务端环境变量中，不提交到 Git 仓库，也不会进入小程序包。

Storage 安全规则属于部署配置的一部分。代码层已经把客户端上传限制为 Backend 生成的 staging File ID，并在 commit 时再次校验，但正式发布前仍需要在 CloudBase 控制台确认客户端只能写 staging 范围，`system/`、`users/` 和 `families/` 等正式资源目录由 Backend 管理。
...
小程序启动后先恢复本地 Token。如果没有有效会话，则进入登录页；用户点击微信登录后，前端通过 `wx.login()` 获取一次性 code，再使用 `wx.cloud.callContainer` 调用 `/api/auth/wechat-login`。Backend 调用微信 `code2Session` 得到 OpenID，根据 OpenID 查找或创建用户并签发 JWT。之后普通业务请求仍然携带 Bearer Token，因此 CloudBase 负责把请求送到正确的 `mealpilot-api`，JWT 负责确认 MealPilot 中的用户身份。

对于第一次登录的用户，系统不会把“身份认证成功”和“个人资料已经完善”混为一件事。如果昵称仍为默认值或头像 File ID 为空，就进入资料完善页；资料保存完成并重新读取会话，确认 `profileComplete=true` 后，才进入推荐、菜单、菜谱和设置等业务页面。

> **图 5-1 登录与首次资料完善运行截图（后续补充）**

## 5.2 微信登录与首次资料完善

登录流程采用“身份确认”和“资料完善”两步设计。OpenID 用来确定“是谁”，昵称和头像用于展示资料，因此昵称头像并不是 `code2Session` 成功的前提。后端通过 `isProfileComplete()` 判断昵称是否仍为默认值，以及头像 File ID 是否已经存在。
...
头像处理采用两阶段上传。资料页拿到本地临时图片路径后，先调用 `/uploads/avatar/prepare`；后端根据当前 JWT 用户生成 `staging/users/<userId>/avatars/<uuid>` 路径，并在 `storage_cleanup_jobs` 中登记 24 小时后的兜底清理时间。随后小程序通过 `wx.cloud.uploadFile` 上传图片，再把返回的 File ID 交给 `/uploads/avatar/commit`。Backend 会检查 File ID 是否属于本次用户的 staging 范围、是否有对应 prepare 记录，再读取 Storage 元数据和文件内容，验证不超过 5MB，并依据 magic bytes 判断是否为 JPEG、PNG 或 WebP。只有校验通过后，文件才会转存到 `users/<userId>/avatars/` 正式目录并写入 `users.avatar_url`。

昵称通过 `PUT /auth/profile` 更新。会话恢复、首次登录和 401 重新认证共用同一套 Auth Service 与 Route Guard；多个并发 401 会复用同一个重新认证过程，原请求最多自动重试一次，避免不同页面各自触发登录。
...
> **图 5-4 推荐条件、候选结果与“换一组”页面截图（后续补充）**
...
云托管容器的本地磁盘不适合作为长期文件存储，因此头像和菜谱封面统一放在 CloudBase 私有 Storage。`users.avatar_url` 和 `recipes.cover_url` 虽然沿用了早期字段名，但当前保存的是稳定的 `cloud://` File ID；页面展示时，后端再批量换取短期 HTTPS URL。

生产上传采用“staging + commit”模式，而不是让客户端直接决定最终文件位置。以菜谱封面为例，前端先调用 `/uploads/recipe-cover/prepare`，Backend 根据当前有效 Family 生成 `staging/families/<familyId>/recipes/<uuid>`；小程序再用 `wx.cloud.uploadFile` 上传本地图片。commit 时，Backend 根据自己的 `CLOUDBASE_FILE_ID_PREFIX` 重新构造允许的 staging 前缀，拒绝跨用户、跨家庭或没有 prepare 记录的 File ID。

后端下载文件前会先读取 Storage 元数据，对超过 5MB 的文件直接拒绝；下载得到 Buffer 后再检查 JPEG、PNG、WebP 的真实文件头。扩展名和客户端 MIME 不作为可信依据。验证通过后，头像转存到 `users/<userId>/avatars/`，菜谱封面转存到 `families/<familyId>/recipes/`，最终扩展名依据真实文件内容确定。

prepare 阶段会同时插入一条 `storage_cleanup_jobs`，`next_attempt_at` 设置为 24 小时后。如果用户上传了 staging 文件却没有完成 commit，后续清理任务仍能删除孤儿文件；commit 成功时则立即删除 staging 文件和对应清理任务。账号或家庭进入最终清理阶段时，正式头像、家庭菜谱封面也会通过同一张任务表异步删除。

这一设计把“传输文件”和“确认业务归属”分开：CloudBase 负责文件上传，Backend 仍掌握最终路径、用户/家庭边界、文件格式和数据库写入。正式发布时还需要配合 Storage 安全规则，仅允许小程序客户端写 staging 范围，正式目录由服务端 SDK 管理。
...
本版本仍处于课程设计和体验验证阶段，没有必须保留的正式用户数据，因此数据库按最新 Schema 重建。`01_schema.sql` 一次建立 20 张业务表，再导入全局基础食材、季节映射和系统标签；清理示例用户和家庭后，再检查表数量、29 条外键、关键字段、基础数据以及孤儿关系。历史迁移脚本继续保留，用于说明结构演化和以后维护已有数据库。

生产代码部署到 CloudBase 云托管。普通业务请求通过 `wx.cloud.callContainer` 访问环境 `cloud1-d1gvr0mwv39a12cbd` 中的 `mealpilot-api`；旧公网地址不再承担生产小程序的主要 API 传输。图片的 prepare/commit 也通过该服务调用，文件本体则使用 `wx.cloud.uploadFile` 写入 Storage staging。开发环境继续使用 localhost HTTP 和 multipart 上传，便于单独调试 Backend。

后端日常运行使用低权限数据库业务账号，只负责正常 CRUD；建库、建表等 DDL 操作使用单独的数据库管理权限。生产环境设置 `NODE_ENV=production`、`DEV_AUTH_ENABLED=false`，同时要求微信登录、JWT 和 Storage 相关配置完整。

## 5.12 测试与验收

项目保留后端直接测试、小程序测试和独立 MySQL 集成测试。Backend 测试覆盖微信认证、家庭数据边界、家庭生命周期、菜谱事务、标签、菜单并发、推荐算法、推荐应用前二次校验、Storage 合约和生产配置；本轮架构调整又增加了 staged upload 测试。小程序测试除页面与登录状态外，还覆盖 `callContainer` 的 Service Header、Bearer JWT、401 重认证，以及 `prepare → wx.cloud.uploadFile → commit` 是否按预期执行。

图片相关测试重点验证四类问题：一是 prepare 生成的路径只能落在当前用户或当前家庭的 staging 范围；二是没有 prepare 记录、跨用户或跨家庭的 File ID 必须被拒绝；三是大于 5MB、非 JPEG/PNG/WebP 的文件不能进入正式目录；四是数据库写入失败时，新生成的正式文件要回滚删除，而 staging 清理任务继续保留作为兜底。

数据库重建完成后的静态验收结果仍为 20 张业务表、29 条外键，基础数据包括 53 项食材、60 项季节映射和 10 个系统标签，示例用户和示例家庭清理后为 0。动态验收则继续覆盖微信登录、资料完善、创建家庭、48 道 Starter Recipe 初始化、推荐生成、候选应用、菜单反馈、统计以及真机图片上传。真实 MySQL 集成测试只在独立测试库和明确写入开关下执行，避免把测试流量写进生产库。

> **图 5-5 家庭管理、菜单、反馈与统计页面截图（后续补充）**
...
最终，微信登录、家庭关系、菜谱与菜单、推荐、反馈以及对象存储已经形成一条完整的数据链。普通业务通过 `callContainer` 进入 Backend，图片由 Storage staging 承担文件传输，再由 Backend 完成业务确认和正式入库。系统中的数据既可以被正常录入和维护，也会继续参与筛选、组合、评分和统计，这使数据库成为业务逻辑的一部分，而不仅仅是数据存放位置。
...
从功能上看，系统已经不只是完成菜谱的增删改查。家庭成员填写的忌口和偏好会影响推荐，历史菜单和评分也会改变后续候选结果，菜单使用情况还可以进一步汇总为 7 天或 30 天统计。数据库中的数据因此能够在后续业务中继续产生作用。工程实现上，生产通信也从默认公网测试域名收敛到 CloudBase 托管调用，并把普通 API 与大文件上传分成两条更合适的通道。
...
目前系统仍然处于课程设计和小规模使用阶段，还有一些边界没有继续扩展。用户一次只能属于一个 active Family，不支持在多个家庭之间自由切换；推荐权重仍然由人工规则设定，还没有根据长期使用数据自动学习；`storage_cleanup_jobs` 已经能够处理 staging 和生命周期文件，但执行仍依赖应用进程，没有独立任务队列和可视化运维界面。

新的图片上传流程虽然把最终业务校验放回 Backend，但客户端能够向哪些 Storage 路径写入仍然受到 CloudBase 安全规则影响。因此正式发布前需要在控制台继续核对并验证：小程序只能创建 staging 文件，不能直接写 `system/`、`users/` 和 `families/` 等正式目录。真实设备端到端测试、云端监控、告警、性能压测和数据库备份恢复演练也还不够完整。

## 6.4 后续完善方向

后续可以在不破坏现有 Family 数据边界的前提下继续扩展多家庭切换、更细的营养目标、长期行为学习、独立后台任务和系统监控。随着真实使用数据增加，还可以分析推荐采纳率、低评分菜谱和家庭菜单习惯，用这些数据调整推荐策略，而不是一开始就引入难以解释的复杂模型。

工程层面可以把 Storage Cleanup 从应用启动后的定时处理逐步拆成独立任务，并为 staging 过期、数据库迁移、推荐运行和 CloudBase 调用建立更完整的监控。课程设计答辩前，还需要把系统功能模块图、总体 E-R 图、总体架构图、菜谱推荐处理流程图以及登录、家庭管理、菜谱、推荐、菜单和统计等运行截图替换到本文对应占位位置。
...
| Family | POST `/families`；POST `/families/join`；GET `/families/current`；PUT/PATCH `/families/current/name`；邀请码刷新、退出、解散、恢复、管理员转移、成员移除 |
...
| Upload（本地兼容） | POST `/uploads/avatar`；POST `/uploads/recipe-cover`，multipart/form-data |
