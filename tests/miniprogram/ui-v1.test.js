...
test('global stylesheet exposes Airbnb color, type, spacing and component tokens', () => {
  ...
  assert.match(css, /--radius-card:\s*var\(--radius-rect\)/i)
...
test('recipe detail uses the confirmed white reading layout and rounded content cards', () => {
  ...
  assert.match(css, /\.detail-description\s*\{[^}]*border-radius:\s*var\(--radius-rect\);[^}]*background:\s*#f7f7f7/s)
  assert.match(css, /\.detail-card\s*\{[^}]*box-sizing:\s*border-box;[^}]*border-radius:\s*var\(--radius-rect\);[^}]*background:\s*#fafafa/s)
...
test('recipe form scheme C uses a safe custom nav, immersive hero and unified editor cards', () => {
  ...
  assert.match(css, /\.form-hero\s*\{[^}]*border-radius:\s*var\(--radius-rect\)/s)
  ...
  assert.match(css, /\.form-nav__title\s*\{[^}]*left:\s*50%;[^}]*font-size:\s*var\(--font-nav-title\);/s)
  ...
  assert.match(css, /\.ingredient-list\s*\{[^}]*border-radius:\s*var\(--radius-rect\);[^}]*background:\s*#fafafa/s)
...
test('settings and about pages use scoped tokens, real actions and custom about navigation', () => {
  ...
  assert.match(settingsCss, /--settings-dashboard: var\(--color-primary-soft\)/)
