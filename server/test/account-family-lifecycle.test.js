...
function accountDatabase(role = 'member', { ownedActiveFamily = false } = {}) {
  const state = {
    users: [{ id: 7, openid: 'member-7', display_name: '成员', avatar_url: 'cloud://env/users/7/avatars/a.jpg' }],
    memberships: [{ id: 70, family_id: 10, user_id: 7, role, nickname: '成员', status: 'active' }],
    jobs: [], calls: [], committed: false, rolledBack: false
  }
  async function execute(sql, params = []) {
    state.calls.push({ sql, params })
    if (/SELECT id, avatar_url FROM users/i.test(sql)) return [[...state.users]]
    if (/SELECT id FROM families WHERE owner_user_id/i.test(sql)) return [ownedActiveFamily ? [{ id: 99 }] : []]
...
test('owner and delegated admin account deletion is rejected without mutation', async () => {
...
test('account deletion cannot orphan an active family when its owner membership row is missing', async () => {
  const database = accountDatabase('member', { ownedActiveFamily: true })
  database.state.memberships = []
  const auth = createAuthService({ database, storageFileIdPrefix: 'cloud://env' })

  await assert.rejects(auth.deleteAccount({ userId: 7 }), (error) => error.code === 'ACCOUNT_ADMIN_BLOCKED')
  assert.equal(database.state.users.length, 1)
  assert.equal(database.state.rolledBack, true)
})
...
function familyDatabase(actorRole = 'owner') {
  const now = new Date()
  const state = {
    families: [{ id: 10, name: '原来的家', invite_code: 'Old123', owner_user_id: 1, status: 'active', disbanded_at: null, purge_after: null }],
    memberships: [
      { id: 11, family_id: 10, user_id: 1, role: 'owner', nickname: '创建者', status: 'active' },
  ...
  async function execute(sql, params = []) {
    ...
    if (/SELECT id, name, disbanded_at AS disbandedAt/i.test(sql)) {
      return [state.families.filter((item) => item.owner_user_id === params[0] && item.status === 'archived').map((item) => ({ id: item.id, name: item.name, disbandedAt: item.disbanded_at, expiresAt: item.purge_after, remainingDays: 30 }))]
    }
    if (/SELECT id, name, invite_code FROM families/i.test(sql)) {
      const family = state.families.find((item) => item.id === Number(params[0]) && item.owner_user_id === Number(params[1]) && item.status === 'archived')
    ...
    if (/UPDATE family_members SET status = 'active', role = 'owner'/i.test(sql)) {
      const member = state.memberships.find((item) => item.id === Number(params[1]) && item.user_id === Number(params[2]))
      if (member) Object.assign(member, { status: 'active', role: 'owner', nickname: params[0] })
...
test('an administrator archives a family, all members leave, and only the owner restores it', async () => {
  ...
  await withServer(familyApp(database, { id: 1, display_name: '创建者' }), async (baseUrl) => {
  ...
  assert.equal(database.state.memberships[0].status, 'active')
  assert.equal(database.state.memberships[1].status, 'left')
