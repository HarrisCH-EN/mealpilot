...
const { loadRecentRecipeUsage } = require('./recent-history')
const { validateSessionPreferences } = require('./preference-matcher')
...
function generateMenuCandidates(input) {
  ...
  const raw = generateRawMenuCandidates({ pools, structure, maxCandidates: MAX_RAW_MENU_CANDIDATES })
  if (!raw.candidates.length) throw new RecommendationDomainError('NO_COMPLETE_MENU', '没有满足结构和家庭限制的完整菜单')
  ...
  const evaluated = raw.candidates.map((candidate) => evaluateMenuCandidate(candidate, context))
  let selected
...
async function generateMenuCandidatesFromDatabase({ connection, database, ...input }) {
  ...
  const [recipes, restrictedIngredientIds, familyCategoryPreferenceScores, recentUsage] = await Promise.all([
...
module.exports = { generateMenuCandidates, generateMenuCandidatesFromDatabase }
