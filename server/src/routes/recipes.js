...
function router({ database, auth, family, mediaUrlService, cloudbaseStorageFileIdPrefix = '' }) {
  ...
  result.put('/recipes/:id', auth, family, asyncRoute(async (request, response) => {
    ...
    if (!rows[0]) throw new HttpError(404, '菜谱不存在')
    if (request.membership.role !== 'admin' && rows[0].author !== request.membership.member_id) throw new HttpError(403, '只能编辑自己创建的菜谱')
    requireFields(request.body, ['title', 'category', 'cookMinutes', 'difficulty'])
    validateRecipeContent(request.body)
  ...
  result.delete('/recipes/:id', auth, family, asyncRoute(async (request, response) => {
    ...
    if (!rows[0]) throw new HttpError(404, '菜谱不存在')
    if (request.membership.role !== 'admin' && rows[0].author !== request.membership.member_id) throw new HttpError(403, '只能删除自己创建的菜谱')
